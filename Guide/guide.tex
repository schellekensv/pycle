...
\begin{abstract}
	This is a beginner-friendly guide to \pycle: a Python toolbox to perform compressive/sketched learning. I first shortly explain what ``compressive learning'' means, and how this toolbox is organized. I then explain, in a detailed tutorial, how you should use (or, who knows, extend?) its main features. If you'd rather dive in right away, I also wrote some practical examples available as jupyter notebooks on \pycle s main github page (\url{https://github.com/schellekensv/pycle}).
\end{abstract}

\section{Introduction}
\subsection{What is compressive learning?}
In usual machine learning, we fit some parameters $\theta$ (e.g., a regression curve, centroids in k-means clustering, weights in a neural network...) to a given set of training data $X$. The actual algorithms for such tasks usually necessitate to access the training set multiple times (one complete pass on the dataset is sometimes called an ``epoch''). This can be cumbersome when the dataset is extremely large, or distributed across different machines. Compressive learning (CL, also called sketched learning) seeks to circumvent this issue by compressing the whole dataset into one lightweight ``sketch'' vector, that requires only one pass on the dataset and can be computed in parallel. The inconveniently large can then be discarded, and learning is done using only this sketch. This allows to significantly reduce the computational resources that are needed to handle massive collections of data.
...
In the following, these steps are intuitively explained, the formal details being introduced only as needed; for a more solid/exhaustive overview of compressive learning, see \cite{gribonval2017compressive}.
...
\begin{enumerate}
	...
	\item We also have some \code{utils.py} module.
	\item We also have a \code{third\_party.py} module.
\end{enumerate}
